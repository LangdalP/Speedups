...
#pragma omp parallel for default(none) shared(line_buffer, imageIn, filterSize) schedule(static)
    for (int y = 0; y < (int)filterSize - 1; ++y) {
        int index = (y * width) * 3;
        int lb_index;
        for (lb_index = 0; lb_index < (int)line_buffer.size(); lb_index += 3) {
    ...
    for (int y = start_coordinate; y < end_y; ++y) {
        ...
        {
            int index = ((y + filterSize / 2) * width) * 3;
            int lb_index;
            for (lb_index = 0; lb_index < (int)line_buffer.size(); lb_index += 3) {
                line_buffer[lb_index] += imageIn[index];
                line_buffer[lb_index + 1] += imageIn[index + 1];
...
#pragma omp parallel for default(none) shared(line_buffer, imageOut)
